...
xdata =pd.read_sql("select * from [PROFUTURO.DW].[dbo].[Informe.Facebook_comments] ",conn)
xdata.head(2)
...
for index, row in xdata.iterrows():
    ...
    cursor.execute("UPDATE [PROFUTURO.DW].[dbo].[Informe.Facebook_comments] SET Mensaje_clean = ? WHERE cod_comment_id = ?", row.filtro_second, row.cod_comment_id)
    cursor.commit()
cursor.close()
