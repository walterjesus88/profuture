...
xdata =pd.read_sql("select * from [PROFUTURO.DW].[dbo].[Informe.Facebook_message] ",conn)
xdata.head(2)
...
xdata['Mensaje'] = xdata['message'].apply(str)
...
xdata = xdata[['Mensaje','id','Mensaje_nuevo']]
xdata
...
for index, row in xdata.iterrows():
    print(row.id)
    cursor.execute("UPDATE [PROFUTURO.DW].[dbo].[Informe.Facebook_message] SET Mensaje_clean = ? WHERE id = ?", row.filtro_second, row.id)
    cursor.commit()
cursor.close()
